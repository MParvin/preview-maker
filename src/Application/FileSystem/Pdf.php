...
class Pdf
{
    ...
    public function setOutputFormat(string $outputFormat)
    {
        if (! $this->isValidOutputFormat($outputFormat)) {
            throw new InvalidFormatException("Format {$outputFormat} is not supported");
        }
     ...
     * @return $this
     *
     * @throws \Module\Application\Exception\InvalidLayerMethod
     *
    ...
    public function setPage(int $page)
    {
        if ($page > $this->getNumberOfPages() || $page < 1) {
            throw new PageDoesNotExistException("Page {$page} does not exist");
        }
